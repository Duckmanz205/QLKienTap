...
export default function LichTrinhDoan_SV() {
  ...
  const trips = [
    ...
    { id: 2, name: 'Acecook Việt Nam - 25/09/2026' }
  ];
  const [selectedTrip, setSelectedTrip] = useState(trips[0]);

  const itinerary = [
    { time: '07:00', task: 'Tập trung tại cơ sở chính HUIT', desc: 'Sinh viên có mặt đúng giờ, điểm danh và xếp hàng lên xe.' },
    { time: '07:30', task: 'Di chuyển đến nhà máy', desc: 'Xe khởi hành. Vui lòng giữ trật tự và vệ sinh chung.' },
    { time: '08:30', task: 'Đến nhà máy Yakult HCM', desc: 'Nghe phổ biến nội quy an toàn lao động và nhận đồ bảo hộ (nếu có).' },
    { time: '09:00', task: 'Tham quan dây chuyền sản xuất', desc: 'Quan sát quy trình sản xuất thực tế dưới sự hướng dẫn của kỹ sư nhà máy.' },
    { time: '10:30', task: 'Hỏi đáp & Giao lưu', desc: 'Sinh viên đặt câu hỏi, đại diện doanh nghiệp giải đáp thắc mắc.' },
    { time: '11:30', task: 'Kết thúc tham quan, lên xe về lại trường', desc: 'Trả lại đồ bảo hộ, tập trung điểm danh lần cuối trước khi lên xe.' },
  ];

  const students = [
    { id: 1, name: 'Nguyễn Văn An', role: 'Đoàn viên' },
    { id: 2, name: 'Trần Thị Bình', role: 'Trưởng nhóm' },
    { id: 3, name: 'Lê Hoàng Cường', role: 'Đoàn viên' },
    { id: 4, name: 'Phạm Duy Khang', role: 'Đoàn viên' },
    { id: 5, name: 'Vũ Quốc Huy', role: 'Đoàn viên' },
    { id: 6, name: 'Hoàng Minh Ngọc', role: 'Đoàn viên' },
    { id: 7, name: 'Đặng Thùy Trâm', role: 'Đoàn viên' },
    { id: 8, name: 'Bùi Gia Bảo', role: 'Đoàn viên' },
  ];
    ...
    <div className="bg-[#E7E0C4]/20 min-h-[calc(100vh-80px)] p-6 animate-in fade-in duration-300 relative" onClick={() => setIsDropdownOpen(false)}>
      ...
      <div className="bg-[#E7E0C4] rounded-2xl p-6 shadow-sm mb-8 flex flex-col lg:flex-row gap-8 lg:items-center justify-between">
        ...
        <div className="flex-1">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-slate-800 mb-4">Nhà máy Yakult HCM</h2>
          
          <div className="space-y-2 text-sm font-medium text-slate-700">
            <div className="flex items-start gap-2.5">
              <MapPin className="w-4 h-4 text-slate-500 mt-0.5 shrink-0" />
              <span className="leading-snug">Số 5 Đại lộ Tự Do, KCN VSIP 1, P. Bình Hòa, TP. Thuận An, Bình Dương</span>
            </div>
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2.5">
                <Calendar className="w-4 h-4 text-slate-500 shrink-0" />
                <span>Thứ Năm, 10/09/2026</span>
              </div>
              <div className="flex items-center gap-2.5">
        ...
        <div className="bg-white/40 rounded-xl p-5 shrink-0 min-w-[280px]">
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-slate-200 border-2 border-white shadow-sm overflow-hidden flex items-center justify-center shrink-0">
              <img src="https://ui-avatars.com/api/?name=Tran+Minh+Triet&background=407F3E&color=fff" alt="Avatar" className="w-full h-full object-cover" />
            </div>
            <div>
              <h4 className="font-bold text-slate-800 text-lg leading-tight">ThS. Trần Minh Triết</h4>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E7E0C4] h-fit">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-6 justify-start">
            {students.map((student) => (
              <div key={student.id} className="flex flex-col items-center w-[72px] group">
